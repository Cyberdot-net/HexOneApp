...
export default (function () {
  ...
  return {
    ...
    sacrificeToken: async (type, amount) => {
      ...
      try {
        const tx = await contract.sacrificeToken(
          Erc20_Tokens_Addr[type].contract,
          amount
        );
        await tx.wait();
        ...
        if (e.error?.message) {
          return {
            status: "failed",
            error: "Sacrifice failed! " + e.error?.message,
          };
        } else if (e.message) {
          return {
            status: "failed",
    ...
    requestAirdrop: async () => {
      ...
      try {
        ...
        if (e.error?.message) {
          return {
            status: "failed",
            error: "Request Airdrop failed! " + e.error?.message,
          };
        } else if (e.message) {
          return {
            status: "failed",
    ...
    claimAirdrop: async () => {
      ...
      try {
        ...
        if (e.error?.message) {
          return {
            status: "failed",
            error: "Claim Airdrop failed! " + e.error?.message,
          };
        } else if (e.message) {
          return {
            status: "failed",
            error:
              "Claim Airdrop failed! " +
              (e.data?.message ? e.data.message : e.message),
          };
        } else {
    ...
    sacrificeInfos: async () => {
      if (!contract) return { status: "failed" };
      const cnt = await contract.sacrificeId();
      let data = [];
      for (let i = 1; i <= cnt; i++) {
        const res = await contract.sacrificeInfos(i);
        data.push(res);
      }
